...
Env:
  ...
  TR_CACHE_PATH   (optional, default data/tr_us_whitelist.csv)
...
def openfigi_batch_isin_to_ticker(isins: list[str]) -> dict[str,str]:
    ...
    def pick(rows):
        # prefer US listings, equity, common/ADR types
...
def main():
    ...
    for isin in isins:
        cus = isin[2:11]
        tk = mapped.get(isin, "")  # no Polygon fallback
        rows.append({"isin": isin, "cusip": cus, "ticker": (tk or "").strip().upper()})

    df = pd.DataFrame(rows).drop_duplicates("cusip")
    # Filter: non-empty, ASCII-safe tickers (regex – kompatibel zu allen pandas)
    # Erlaubt: A-Z, 0-9, Punkt, Bindestrich (typisch BRK.B / RDS-A etc.)
